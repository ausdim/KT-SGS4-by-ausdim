...
# Kernel Tuning by Ghost44. Thanks to Dorimanx.
...
PIDOFINIT=$(pgrep -f "/sbin/ext/ghost.sh");
...
if [ ! -d /data/.ghost ]; then
	$BB mkdir -p /data/.ghost;
fi;

$BB chmod -R 0777 /data/.ghost/;

. /res/customconfig/customconfig-helper;

ccxmlsum=`md5sum /res/customconfig/customconfig.xml | awk '{print $1}'`
if [ "a${ccxmlsum}" != "a`cat /data/.ghost/.ccxmlsum`" ];
then
   $BB rm -f /data/.ghost/*.profile;
   echo ${ccxmlsum} > /data/.ghost/.ccxmlsum;
fi;

[ ! -f /data/.ghost/default.profile ] && cp /res/customconfig/default.profile /data/.ghost/default.profile;
[ ! -f /data/.ghost/battery.profile ] && cp /res/customconfig/battery.profile /data/.ghost/battery.profile;
[ ! -f /data/.ghost/balanced.profile ] && cp /res/customconfig/balanced.profile /data/.ghost/balanced.profile;
[ ! -f /data/.ghost/performance.profile ] && cp /res/customconfig/performance.profile /data/.ghost/performance.profile;
...
if [ -f /system/app/STweaks.apk ] || [ -f /data/app/STweaks.apk ] ; then
	...
	$BB cp /res/GTweaks.apk /system/app/;
	$BB chown root.root /system/app/GTweaks.apk;
	$BB chmod 644 /system/app/GTweaks.apk;
fi;

if [ ! -f /system/app/GTweaks.apk ] ; then
	$BB rm -f /system/app/STweaks.apk > /dev/null 2>&1;
	$BB rm -f /data/app/STweaks.apk > /dev/null 2>&1;
	$BB rm -f /system/app/GTweaks.apk > /dev/null 2>&1;
	$BB rm -f /data/app/com.gokhanmoral.stweaks* > /dev/null 2>&1;
